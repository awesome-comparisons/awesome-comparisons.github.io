---
title: Type Checking
description: How and when type correctness is verified across different programming languages.
---
...
Dart is statically and soundly typed, with type inference reducing the need for explicit annotations and null safety built into the type system so nullable and non-null types are distinct; all checks happen at compile time by the analyzer and compiler front end.
...
Perl is dynamically typed with essentially no static type checking; scalars, arrays, and hashes hold whatever data is assigned to them, and type-related errors—like calling a method that doesn't exist—only surface at runtime.
...
Scala has a strong static type system with extensive type inference, checked at compile time by `scalac`; features like variance annotations, implicits/given instances, and path-dependent types let the compiler enforce sophisticated constraints beyond simple type matching.

## Clojure

Clojure is dynamically typed by default, resolving most type mismatches at runtime; optional gradual typing is available through `clojure.spec` for data validation or third-party tools like `core.typed`, but neither is enforced by the compiler itself.

## Haskell

Haskell has a strong static type system based on Hindley-Milner-style inference, so most code needs no explicit type annotations while still being fully checked at compile time; typeclasses add further compile-time-checked polymorphism and constraint solving.

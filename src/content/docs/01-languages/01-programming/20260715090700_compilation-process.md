...
description: How source code moves from text to executable form across different programming languages.
---
...
Dart AOT-compiles to native machine code for release builds (used for mobile and desktop apps via `dart compile exe`), while during development the Dart VM JIT-compiles and supports hot reload for fast iteration; Dart also compiles to JavaScript via dart2js for web deployment.
...
Perl has no separate build step—the interpreter parses source and compiles it to an internal bytecode-like op tree just before execution, then walks that tree to run the program, all within a single invocation.
...
The `scalac` compiler performs type inference and checking, applies macro expansion, then emits JVM bytecode (or, via Scala.js and Scala Native, JavaScript or native code) that runs on the JVM alongside ordinary Java classes.

## Clojure

Clojure source is read by the reader into data structures, macro-expanded, and compiled to JVM bytecode at load time—there's no separate ahead-of-time build step by default—with forms typically compiled and evaluated one at a time in a running JVM process.

## Haskell

GHC desugars Haskell source into a small Core language, runs a series of type-directed optimization passes over Core, then lowers it through the STG and Cmm intermediate representations to native machine code, producing a standalone compiled binary.
